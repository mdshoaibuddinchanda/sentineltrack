...
def main() -> int:
    ...
    final_selection = ["# Final Model Selection", "", "Selection is based on measured evidence, not parameter count:", "", "- OCR: PP-OCRv5 mobile remains selected for the balanced profile because its expanded-validation exact ties server after postprocessing while retaining substantially lower latency and better character accuracy/CER.", "- Temporal: current voter with a 5-frame window is the balanced operational choice; 8-frame current voter has the highest measured exact on its smaller eligible subset.", "- P3: the trained YOLO11s candidate is selected provisionally over production on the strict test because its F1 and recall are higher while retaining real-time GPU throughput; final profile promotion follows the complete tournament.", "- P1: retain YOLO11m vehicle model as operational baseline until a real vehicle GT corpus is supplied."]
