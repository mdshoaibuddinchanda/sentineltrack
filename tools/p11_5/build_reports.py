...
def main() -> int:
    ...
    detector_files = sorted(eval_dir.glob("*.json")) if eval_dir.exists() else []
    ...
    p3_lines = ["# P3 Tournament", "", "## Outcome", "", "The candidate runs below are measured on the strict detection V2 test split. Production weights were never overwritten.", "", table(["run", "imgsz", "P", "R", "F1", "mAP50", "mAP50-95", "tiny R", "square/tall R"], [[r.get("run_id"), r.get("imgsz"), r.get("precision"), r.get("recall"), r.get("f1"), r.get("map50"), r.get("map50_95"), r.get("subsets", {}).get("tiny_lt60", {}).get("recall"), r.get("subsets", {}).get("square_or_tall", {}).get("recall")] for r in detector_rows] or [["no completed candidate", "-", "-", "-", "-", "-", "-", "-", "-"]]), "", "## Required candidates and blockers", "", "YOLO11s transfer was run locally. YOLO11m transfer is scheduled after the first run. YOLO26 was not present in the installed Ultralytics 8.3.235 model/config package; no unsupported checkpoint was fabricated. YOLO11s/YOLO11m OBB support is handled separately in OBB_REPORT.md.", "", f"Historical production baseline reference: {baseline.get('f1_score', baseline.get('f1', 'not available'))} F1 on the earlier canonical test; it is not directly interchangeable with the strict derivative test." ]
    write("P3_TOURNAMENT.md", "\n".join(p3_lines))

    obb = read_json(REPORT_DIR / "dataset" / "OBB_V2_STRICT_FREEZE.json", {})
    obb_train = [read_json(path, {}) for path in (ROOT / "runs" / "p11_5").glob("*/train_result.json") if "obb" in path.parent.name.lower()] if (ROOT / "runs" / "p11_5").exists() else []
    obb_eval_dir = REPORT_DIR / "obb_eval"
    obb_eval = [read_json(path, {}) for path in obb_eval_dir.glob("*.json") if not path.name.endswith("_dataset.yaml")] if obb_eval_dir.exists() else []
    ...
    final_selection = ["# Final Model Selection", "", "Selection is based on measured evidence, not parameter count:", "", "- OCR: PP-OCRv5 mobile remains selected for the balanced profile because its expanded-validation exact ties server after postprocessing while retaining substantially lower latency and better character accuracy/CER.", "- Temporal: current voter with a 5-frame window is the balanced operational choice; 8-frame current voter has the highest measured exact on its smaller eligible subset.", "- P3: select the candidate with highest locked-test recall/F1 subject to latency and tiny/square subset guardrails after the completed detector leaderboard is available.", "- P1: retain YOLO11m vehicle model as operational baseline until a real vehicle GT corpus is supplied."]
    write("FINAL_MODEL_SELECTION.md", "\n".join(final_selection))

    write("FINAL_REPORT.md", "\n".join(["# SentinelTrack P11.5B–E Final Execution Report", "", "## Executive summary", "", "This branch contains measured local P11.5 work: strict dataset freezes, a true multiframe benchmark, isolated detector/OBB harnesses, OCR screening, temporal consensus, preprocessing sweeps, synthetic corpus generation, and operational reporting. Frozen V1 datasets, frontend, and CI were not modified.", "", "## Measured outcomes", "", f"- Detector candidate reports: {len(detector_rows)} evaluation artifacts.", f"- OCR candidate rows: {len(ocr_rows)}.", f"- Temporal tracks: {temporal.get('tracks', 'not run')} across {temporal.get('frames', 'not run')} crops.", f"- Synthetic corpus: {synthetic.get('count', 0)} generated examples against a 100,000 target.", "", "## Hard blockers and limitations", "", "- YOLO26 is unavailable in the installed local Ultralytics package; official OBB weights were downloaded only for the supported YOLO11 OBB stage.", "- No external vehicle GT corpus was available, so P1 recall/FPR and P5 safety regression are not claimable.", "- OCR fine-tuning was not completed because compatible training/export dependencies and modern local checkpoints are missing.", "- Synthetic 25%/50% ablation training remains pending; synthetic data is not used for authoritative test claims.", "- Cross-split raw SHA and identity leakage are clean. Upstream detection V2 retains pHash-near review findings; the strict derivative removes exact cross-split pHash source copies while preserving canonical V1 assignments.", "", "## Reproducibility", "", "Use the PY312 interpreter, the committed tools under tools/p11_5, the recorded manifest hashes, and the run registry. Candidate weights remain outside Git under runs/p11_5; production weights are never overwritten." ]))
    return 0
