"""Evaluate multiframe OCR consensus on the locked real sequence benchmark."""
...
def main() -> int:
    ...
    temporal_mod = importlib.import_module("experiments.archive.p11_5.temporal")
    quality_mod = importlib.import_module("experiments.archive.p11_5.quality")
    ...
    for track_id, items in by_track.items():
        ...
        for row in items:
            ...
            quality = quality_mod.crop_quality(image)
            values.append({
                "text": text or "",
    ...
    for window in (1, 3, 5, 8):
        methods = {"single_best": [], "current_voter": [], "weighted_vote": [], "character_fusion": []}
        truths: list[str] = []
        eligible = 0
        for items in observations.values():
            ...
            best = temporal_mod.best_observation(sample)
            methods["single_best"].append(best["text"] if best else "")
            weighted = temporal_mod.temporal_vote(sample, min_support=1)
            methods["weighted_vote"].append(weighted.get("selected_text") or "")
            candidates = [(item["text"], max(0.01, item["confidence"] * item["quality"])) for item in sample if item["text"]]
            methods["character_fusion"].append(voter_mod.resolve_character_consensus(candidates))
            hypotheses = []
            for index, item in enumerate(sample):
                hypotheses.append(models_mod.OCRHypothesis(
                    ...
                    character_confidences=item.get("char_confidences", []),
                    plate_width=0, plate_height=0,
                ))
            voted = voter_mod.MultiFramePlateVoter(min_support_count=1).vote(hypotheses)
            methods["current_voter"].append(voted.best_text or "")
        for method, predictions in methods.items():
            rows_out.append({"window": window, "method": method, "eligible_tracks": eligible, **metric(predictions, truths, eval_mod.weighted_levenshtein, eval_mod.normalize_plate_text)})
    ...
    output = {
        ...
        "inference_latency_ms": {
            "p50": round(statistics.median(inference_latencies), 3) if inference_latencies else None,
            "p95": round(sorted(inference_latencies)[max(0, int(len(inference_latencies) * 0.95) - 1)], 3) if inference_latencies else None,
        },
        "logit_fusion": {"status": "NOT_AVAILABLE", "reason": "The current ONNX recognizer exposes decoded text/character confidence, not per-timestep logits."},
        "results": rows_out,
    }
    REPORT_DIR.mkdir(parents=True, exist_ok=True)
    (REPORT_DIR / "temporal_evaluation.json").write_text(json.dumps(output, indent=2), encoding="utf-8")
    with (REPORT_DIR / "temporal_leaderboard.csv").open("w", encoding="utf-8", newline="") as handle:
        fields = ["window", "method", "eligible_tracks", "tracks", "exact_matches", "exact_accuracy", "character_accuracy", "cer"]
