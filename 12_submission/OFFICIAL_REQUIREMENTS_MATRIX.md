...
| Onboard approximately 50 simulated government feeds | FAQ Q26, Q39–41 | Working test-case run and evidence | P0 catalogue client; 30 persisted official source definitions; `DEMO_RUNBOOK.md` | PARTIAL | Run against the organizer feed; the current protected catalogue returns `AUTH_REQUIRED` locally and the feed count is dynamic |
...
| Provide complete route and location/time movement history | FAQ Q28 | Output report with timestamps and locations | P7 trajectory/GeoJSON APIs; authenticated CSV report; `HLD.md` | PARTIAL | Export challenge run as evidence; current local registry has textual location labels but no verified coordinates |
...
| Approximately 12 hours from 30+ cameras across five departments | FAQ Q39 | Readiness for provided data | 30-camera registry; `HLD.md`; `DEMO_RUNBOOK.md` | PARTIAL | Obtain actual catalogue credentials and department/GIS metadata from organizer |
